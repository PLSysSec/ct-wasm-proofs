...
lemma store_preserved1:
  assumes "\<lparr>s;vs;es\<rparr> a\<leadsto>_i \<lparr>s';vs';es'\<rparr>"
          "store_typing s \<S>"
          "\<S>\<bullet>\<C> \<turnstile> es : (ts _> ts')"
          "\<C> = ((s_inst \<S>)!i)\<lparr>trust_t := tr, local := local ((s_inst \<S>)!i) @ (map typeof vs), label := arb_label, return := arb_return\<rparr>"
          "i < length (s_inst \<S>)"
  shows "store_typing s' \<S>"
  using assms
proof (induction arbitrary: \<C> tr arb_label arb_return ts ts' rule: reduce.induct)
  case (callcl_host_Some cl tr t1s t2s f ves vcs n m s i s' vcs' vs)
...
next
  ...
  obtain tls where t_local:"(\<S>\<bullet>((s_inst \<S>)!i)\<lparr>trust_t:= trust_t \<C>, local := (local ((s_inst \<S>)!i)) @ (map typeof vs), return := Some tls\<rparr> \<turnstile> es : ([] _> tls))"
...
lemma store_preserved:
  assumes "\<lparr>s;vs;es\<rparr> a\<leadsto>_i \<lparr>s';vs';es'\<rparr>"
          "store_typing s \<S>"
          "\<S>\<bullet>tr\<bullet>None \<tturnstile>_i vs;es : ts"
  shows "store_typing s' \<S>"
proof -
  show ?thesis
...
lemma typeof_cvtop:
  ...
  shows "(typeof v) = t"
        "cvtop = Convert \<Longrightarrow> (t1 \<noteq> t) \<and> (t_sec t1 = t_sec t) \<and> ((sx = None) = ((is_float_t t1 \<and> is_float_t t) \<or> (is_int_t t1 \<and> is_int_t t \<and> (t_length t1 < t_length t))))"
        "cvtop = Reinterpret \<Longrightarrow> (t1 \<noteq> t) \<and> t_sec t1 = t_sec t \<and> t_length t1 = t_length t"
        "cvtop = Classify \<Longrightarrow> is_int_t t \<and> is_public_t t \<and> classify_t t = t1"
        "cvtop = Declassify \<Longrightarrow> (trust_t \<C>) = Trusted \<and> is_int_t t \<and> is_secret_t t \<and> declassify_t t = t1"
proof -
  have  "\<C> \<turnstile> [C v, e] : (ts _> ts')"
    using unlift_b_e assms(1)
    by simp
...
lemma typeof_callcl_host:
  assumes "\<S>\<bullet>\<C> \<turnstile> ($$* vs) @ [e] : (ts _> ts')"
          "e = Callcl cl"
          "cl = Func_host (tr,tf) f"
  shows "trust_compat (trust_t \<C>) tr"
proof -
    obtain ts'' where ts'_def:"\<S>\<bullet>\<C> \<turnstile> $$* vs: (ts _> ts'')" "\<S>\<bullet>\<C> \<turnstile> [e] : (ts'' _> ts')"
      using e_type_comp[OF assms(1)]
      by fastforce
    thus ?thesis
      using assms(2,3) e_type_callcl
      unfolding cl_type_def
      by fastforce
qed

lemma types_preserved_unop_testop_cvtop:
  assumes "\<lparr>[$C v, $e]\<rparr> a\<leadsto> \<lparr>[$C v']\<rparr>"
...
proof -
  ...
  have "arity_1_result e = typeof (v')"
    ...
    apply (cases rule: reduce_simple.cases)
           ...
           apply (auto simp add: typeof_def t_sec_def classify_t_classify_typeof declassify_t_declassify_typeof)
    done
...
lemma typeof_binop_relop:
  ...
  shows "typeof v1 = t"
        "typeof v2 = t"
        "e = Binop_i t iop \<Longrightarrow> is_secret_t t \<Longrightarrow> safe_binop_i iop"
...
lemma types_preserved_binop_relop:
  assumes "\<lparr>[$C v1, $C v2, $e]\<rparr> a\<leadsto> \<lparr>[$C v']\<rparr>"
...
proof -
  ...
  have "arity_2_result e = typeof (v')"
    using assms(1,3)
    by (cases rule: reduce_simple.cases) (auto simp add: arity_2_result_def typeof_def t_sec_def)
...
lemma types_preserved_drop:
  assumes "\<lparr>[$C v, $e]\<rparr> a\<leadsto> \<lparr>[]\<rparr>"
          "\<S>\<bullet>\<C> \<turnstile> [$C v, $e] : (ts _> ts')"
          "(e = (Drop))"
...
lemma typeof_select:
  assumes "\<S>\<bullet>\<C> \<turnstile> [$C v1, $C v2, $C vn, $e] : (ts _> ts')"
          "(e = Select sec)"
  shows "t_sec (typeof vn) = sec"
        "typeof v1 = typeof v2"
...
proof -
  ...
  then obtain t1s where t1s_def:"\<C> \<turnstile> [C v1, C v2, C vn] : (ts _> t1s)" "\<C> \<turnstile> [e] : (t1s _> ts')"
    ...
    by fastforce
  then obtain t2s t where t2s_def:"t1s = t2s @ [t, t, (T_i32 sec)]"
                                  "ts' = t2s@[t]"
                                  "(sec = Secret \<longrightarrow> is_secret_t t)"
    using b_e_type_select[of \<C> e t1s] assms
    by fastforce
  thus "sec = Secret \<longrightarrow> is_secret_t (typeof v1)"
       "t_sec (typeof vn) = sec"
       "typeof v1 = typeof v2"
       "sec = Secret \<longrightarrow> is_secret_t (typeof v2)"
    using t1s_def t2s_def b_e_type_value_list[of \<C> "[C v1, C v2]" "vn" ts "t2s@[t,t]"] t_sec_def
          b_e_type_value2[of \<C> v1 v2]
    by fastforce+
    
qed

lemma types_preserved_select:
  assumes "\<lparr>[$C v1, $C v2, $C vn, $e]\<rparr> a\<leadsto> \<lparr>[$C v3]\<rparr>"
...
proof -
  ...
  then obtain t1s where t1s_def:"\<C> \<turnstile> [C v1, C v2, C vn] : (ts _> t1s)" "\<C> \<turnstile> [e] : (t1s _> ts')"
    using b_e_type_comp[where ?e = e and ?es = "[C v1, C v2, C vn]"]
    by fastforce
  then obtain t2s t sec where t2s_def:"t1s = t2s @ [t, t, (T_i32 sec)]" "ts' = t2s@[t]"
    using b_e_type_select[of \<C> e t1s] assms
    by fastforce
...
lemma types_preserved_block:
  assumes "\<lparr>vs @ [$Block (tn _> tm) es]\<rparr> a\<leadsto> \<lparr>[Label m [] (vs @ ($* es))]\<rparr>"
...
lemma typeof_if:
  assumes "\<S>\<bullet>\<C> \<turnstile> [$C ConstInt32 sec n, $If tf e1s e2s] : (ts _> ts')"
  shows "sec = Public"
proof -
  have "\<C> \<turnstile> [C ConstInt32 sec n, If tf e1s e2s] : (ts _> ts')"
    using unlift_b_e assms
    by fastforce
  then obtain ts_i where ts_i_def:"\<C> \<turnstile> [C ConstInt32 sec n] : (ts _> ts_i)" "\<C> \<turnstile> [If tf e1s e2s] : (ts_i _> ts')"
    using b_e_type_comp
    by (metis append_Cons append_Nil)
  thus ?thesis
    using b_e_type_if[OF ts_i_def(2)] b_e_type_value[OF ts_i_def(1)]
    unfolding typeof_def
    by fastforce
qed

lemma types_preserved_if:
  assumes "\<lparr>[$C ConstInt32 sec n, $If tf e1s e2s]\<rparr> a\<leadsto> \<lparr>[$Block tf es']\<rparr>"
          "\<S>\<bullet>\<C> \<turnstile> [$C ConstInt32 sec n, $If tf e1s e2s] : (ts _> ts')"
...
lemma types_preserved_tee_local:
  assumes "\<lparr>[v, $Tee_local i]\<rparr> a\<leadsto> \<lparr>[v, v, $Set_local i]\<rparr>"
...
lemma types_preserved_loop:
  assumes "\<lparr>vs @ [$Loop (t1s _> t2s) es]\<rparr> a\<leadsto> \<lparr>[Label n [$Loop (t1s _> t2s) es] (vs @ ($* es))]\<rparr>"
...
lemma types_preserved_label_value:
  assumes "\<lparr>[Label n es0 vs]\<rparr> a\<leadsto> \<lparr>vs\<rparr>"
...
lemma typeof_br_if:
  assumes "\<S>\<bullet>\<C> \<turnstile> [$C ConstInt32 sec n, $Br_if i] : (ts _> ts')"
  shows "sec = Public"
proof -
  have "\<C> \<turnstile> [C ConstInt32 sec n, Br_if i] : (ts _> ts')"
    using unlift_b_e assms(1)
    by fastforce
  then obtain ts'' where ts''_def:"\<C> \<turnstile> [C ConstInt32 sec n] : (ts _> ts'')" "\<C> \<turnstile> [Br_if i] : (ts'' _> ts')"
  using b_e_type_comp[of _ "[C ConstInt32 sec n]" "Br_if i"]
  by fastforce
  thus ?thesis
    using b_e_type_br_if[of \<C> "Br_if i" ts'' ts' i] b_e_type_value[OF ts''_def(1)]
    unfolding typeof_def
    by fastforce
qed

lemma types_preserved_br_if:
  assumes "\<lparr>[$C ConstInt32 sec n, $Br_if i]\<rparr> a\<leadsto> \<lparr>e\<rparr>"
          "\<S>\<bullet>\<C> \<turnstile> [$C ConstInt32 sec n, $Br_if i] : (ts _> ts')"
          "e = [$Br i] \<or> e = []"
...
lemma typeof_br_table:
  assumes "\<S>\<bullet>\<C> \<turnstile> [$C ConstInt32 sec c, $Br_table is i] : (ts _> ts')"
  shows "sec = Public"
proof -
  have "\<C> \<turnstile> [C ConstInt32 sec c, Br_table is i] : (ts _> ts')"
    using unlift_b_e assms(1)
    by fastforce
  then obtain ts'' where ts''_def:"\<C> \<turnstile> [C ConstInt32 sec c] : (ts _> ts'')" "\<C> \<turnstile> [Br_table is i] : (ts'' _> ts')"
    using b_e_type_comp[of _ "[C ConstInt32 sec c]" "Br_table is i"]
    by fastforce
  thus ?thesis
    using b_e_type_br_table[of \<C> "Br_table is i" ts'' ts'] b_e_type_value
    unfolding typeof_def
    by fastforce
qed

lemma types_preserved_br_table:
  assumes "\<lparr>[$C ConstInt32 sec c, $Br_table is i]\<rparr> a\<leadsto> \<lparr>[$Br i']\<rparr>"
          "\<S>\<bullet>\<C> \<turnstile> [$C ConstInt32 sec c, $Br_table is i] : (ts _> ts')"
...
lemma types_preserved_local_const:
  assumes "\<lparr>[Local n i vs es]\<rparr> a\<leadsto> \<lparr>es\<rparr>"
          "\<S>\<bullet>\<C> \<turnstile> [Local n i vs es] : (ts _> ts')"
          "const_list es"
  shows "\<S>\<bullet>\<C> \<turnstile> es: (ts _> ts')"
proof -
  obtain tls where "(\<S>\<bullet>((s_inst \<S>)!i)\<lparr>trust_t := (trust_t \<C>), local := (local ((s_inst \<S>)!i)) @ (map typeof vs), return := Some tls\<rparr> \<turnstile> es : ([] _> tls))"
                   "ts' = ts @ tls"
...
lemma types_preserved_call_indirect_Some:
  assumes "\<S>\<bullet>\<C> \<turnstile> [$C ConstInt32 sec c, $Call_indirect j] : (ts _> ts')"
...
proof -
  ...
  obtain ts'' where ts''_def:"\<C> \<turnstile> [C ConstInt32 sec c] : (ts _> ts'')"
                             "\<C> \<turnstile> [Call_indirect j] : (ts'' _> ts')"
    using e_type_comp[of \<S> \<C> "[$C ConstInt32 sec c]" "$Call_indirect j" ts ts']
          assms(1)
          unlift_b_e[of \<S> \<C> "[C ConstInt32 sec c]"]
          unlift_b_e[of \<S> \<C> "[Call_indirect j]"]
    by fastforce
  hence "ts'' = ts@[(T_i32 sec)]"
    using b_e_type_value
    unfolding typeof_def
    by fastforce
  moreover
  ...
  hence stypes_eq:"types_t (s_inst \<S> ! i') = types (inst s ! i')"
    ...
    by fastforce
  obtain ts''a where ts''a_def:"trust_compat (trust_t \<C>) tr'"
                               "j < length (types_t \<C>)"
                               "ts'' = ts''a @ t1s @ [(T_i32 Public)]"
                               "ts' = ts''a @ t2s"
                               "types_t \<C> ! j = (tr',(t1s _> t2s))"
...
lemma types_preserved_call_indirect_None:
  ...
  shows "\<S>\<bullet>\<C> \<turnstile> [Trap] : (ts _> ts')"
        "sec = Public"
proof -
  show "\<S>\<bullet>\<C> \<turnstile> [Trap] : (ts _> ts')"
...
proof -
  ...
  then obtain tvs where tvs_def:"ts'' = ts @ tvs"
    ...
    by fastforce    
  obtain ts_c \<C>' where ts_c_def:"trust_compat (trust_t \<C>) tr"
                                "(ts'' = ts_c @ t1s)"
                                "(ts' = ts_c @ t2s)"
                                "i < length (s_inst \<S>)"
                                "\<C>' = ((s_inst \<S>)!i)"
                                "(\<C>'\<lparr>trust_t := tr, local := (local \<C>') @ t1s @ tfs, label := ([t2s] @ (label \<C>')), return := Some t2s\<rparr>  \<turnstile> es : ([] _> t2s))"
  ...
  have "inst_typing \<S> (inst s ! i) (s_inst \<S> ! i)"
    using store_typing_imp_inst_length_eq[OF assms(9)] store_typing_imp_inst_typing[OF assms(9)]
          ts_c_def(4)
    by simp
  obtain \<C>'' where c''_def:"\<C>'' = \<C>'\<lparr>trust_t := tr, local := (local \<C>') @ t1s @ tfs, return := Some t2s\<rparr>"
    by blast
  hence "\<C>''\<lparr>label := ([t2s] @ (label \<C>''))\<rparr>  = \<C>'\<lparr>trust_t := tr, local := (local \<C>') @ t1s @ tfs, label := ([t2s] @ (label \<C>')), return := Some t2s\<rparr>"
    by fastforce
...
lemma types_preserved_return:
  assumes "\<lparr>[Local n i vls LI]\<rparr> a\<leadsto> \<lparr>ves\<rparr>"
...
proof -
  obtain tls \<C>' where l_def:"i < length (s_inst \<S>)"
                        "\<C>' = ((s_inst \<S>)!i)\<lparr>trust_t := (trust_t \<C>), local := (local ((s_inst \<S>)!i)) @ (map typeof vls), return := Some tls\<rparr>"
...
lemma types_preserved_br:
  assumes "\<lparr>[Label n es0 LI]\<rparr> a\<leadsto> \<lparr>vs @ es0\<rparr>"
...
lemma types_preserved_b_e1:
  assumes "\<lparr>es\<rparr> a\<leadsto> \<lparr>es'\<rparr>"
          "store_typing s \<S>"
          "\<S>\<bullet>\<C> \<turnstile> es : (ts _> ts')"
  shows "\<S>\<bullet>\<C> \<turnstile> es' : (ts _> ts')"
  using assms(1)
...
next
  ...
  thus ?thesis
    ...
    by simp
next
  case (binop_i32_Some v iop c1 c2)
  thus ?thesis
...
next
  case (binop_i64_Some v iop c1 c2)
  thus ?thesis
    using assms(1, 3) types_preserved_binop_relop
    by simp
next
  ...
  thus ?thesis
    by (simp add: e_typing_s_typing.intros(4))
next
  case (binop_f32_Some v fop c1 c2)
  thus ?thesis
    using assms(1, 3) types_preserved_binop_relop
    by simp
next
  ...
  thus ?thesis
    by (simp add: e_typing_s_typing.intros(4))
next
  case (binop_f64_Some v fop c1 c2)
  then show ?thesis
    using assms(1, 3) types_preserved_binop_relop
    by simp
next
...
next
  ...
  then show ?thesis
    ...
    by simp
next
  case (convert_Some t1 v v' t2 sx)
  then show ?thesis
    using assms(1, 3) types_preserved_unop_testop_cvtop
    by simp
next
...
lemma types_preserved_b_e:
  assumes "\<lparr>es\<rparr> a\<leadsto> \<lparr>es'\<rparr>"
          "store_typing s \<S>"
          "\<S>\<bullet>tr\<bullet>None \<tturnstile>_i vs;es : ts"
  shows "\<S>\<bullet>tr\<bullet>None \<tturnstile>_i vs;es' : ts"
proof -
  have "i < (length (s_inst \<S>))"
    using assms(3) s_typing.cases
    by blast
  moreover
  obtain tvs \<C> where defs: "tvs = map typeof vs" "\<C> = ((s_inst \<S>)!i)\<lparr>trust_t := tr, local := (local ((s_inst \<S>)!i) @ tvs), return := None\<rparr>" "\<S>\<bullet>\<C> \<turnstile> es : ([] _> ts)"
    using assms(3)
...
lemma types_preserved_store:
  assumes "\<S>\<bullet>\<C> \<turnstile> [$C ConstInt32 sec k, $C v, $Store t tp a off] : (ts _> ts')"
  shows "\<S>\<bullet>\<C> \<turnstile> [] : (ts _> ts')"
        "sec = Public"
        "types_agree t v"
proof -
  obtain ts'' ts''' where ts_def:"\<S>\<bullet>\<C> \<turnstile> [$C ConstInt32 sec k] : (ts _> ts'')"
                                 "\<S>\<bullet>\<C> \<turnstile> [$C v] : (ts'' _> ts''')"
                                 "\<S>\<bullet>\<C> \<turnstile> [$Store t tp a off] : (ts''' _> ts')"
    using assms e_type_comp_conc2[of \<S> \<C> "[$C ConstInt32 sec k]" "[$C v]" "[$Store t tp a off]"]
    by fastforce
  then have "ts'' = ts@[(T_i32 sec)]"
    using b_e_type_value[of \<C> "C ConstInt32 sec k" "ts" ts'']
          unlift_b_e[of \<S> \<C> "[C (ConstInt32 sec k)]" "(ts _> ts'')"]
    unfolding typeof_def
    by fastforce
  hence "ts''' = ts@[(T_i32 sec), (typeof v)]"
    using ts_def(2) b_e_type_value[of \<C> "C v" ts'' ts''']
          unlift_b_e[of \<S> \<C> "[C v]" "(ts'' _> ts''')"]
    by fastforce
  hence "ts = ts'" "sec = Public" "types_agree t v"
    ...
    by fastforce+
  thus "\<S>\<bullet>\<C> \<turnstile> [] : (ts _> ts')" "sec = Public" "types_agree t v"
...
lemma types_preserved_current_memory:
  ...
  shows "\<S>\<bullet>\<C> \<turnstile> [$C ConstInt32 Public c] : (ts _> ts')"
proof -
  have "ts' = ts@[(T_i32 Public)]"
    using assms b_e_type_current_memory unlift_b_e[of \<S> \<C> "[Current_memory]"]
    by fastforce
  thus ?thesis
    using b_e_typing.const[of \<C> "ConstInt32 Public c"] e_typing_s_typing.intros(1,3)
    unfolding typeof_def
    by fastforce
qed

lemma types_preserved_grow_memory:
  assumes "\<S>\<bullet>\<C> \<turnstile> [$C ConstInt32 sec c, $Grow_memory] : (ts _> ts')"
  shows "\<S>\<bullet>\<C> \<turnstile> [$C ConstInt32 sec c'] : (ts _> ts')"
        "sec = Public"
proof -
  obtain ts'' where ts''_def:"\<S>\<bullet>\<C> \<turnstile> [$C ConstInt32 sec c] : (ts _> ts'')"
  ...
  have "ts'' = ts@[(T_i32 sec)]"
    using b_e_type_value[of \<C> "C ConstInt32 sec c" ts ts'']
          unlift_b_e[of \<S> \<C> "[C ConstInt32 sec c]"] ts''_def(1)
    unfolding typeof_def
    by fastforce
  moreover
  hence "ts'' = ts'" and "sec = Public"
    ...
    by fastforce+
  ultimately
  show "\<S>\<bullet>\<C> \<turnstile> [$C ConstInt32 sec c'] : (ts _> ts')" "sec = Public"
    using e_typing_s_typing.intros(1,3)
          b_e_typing.const[of \<C> "ConstInt32 sec c'"]
    unfolding typeof_def
    by fastforce+
qed
...
lemma types_preserved_load:
  assumes "\<S>\<bullet>\<C> \<turnstile> [$C ConstInt32 sec k, $Load t tp a off] : (ts _> ts')"
          "typeof v = t"
  shows "\<S>\<bullet>\<C> \<turnstile> [$C v] : (ts _> ts')"
        "sec = Public"
proof -
  obtain ts'' where ts''_def:"\<S>\<bullet>\<C> \<turnstile> [$C ConstInt32 sec k] : (ts _> ts'')"
                             "\<S>\<bullet>\<C> \<turnstile> [$Load t tp a off] : (ts'' _> ts')"
    using e_type_comp assms
    by (metis append_butlast_last_id butlast.simps(2) last.simps list.distinct(1))
  hence "ts'' = ts@[(T_i32 sec)]"
    using b_e_type_value unlift_b_e[of \<S> \<C> "[C ConstInt32 sec k]"]
    unfolding typeof_def
    by fastforce
  hence ts_def:"sec = Public" "ts' = ts@[t]" "load_store_t_bounds a (option_projl tp) t"
    using ts''_def(2) b_e_type_load unlift_b_e[of \<S> \<C> "[Load t tp a off]"]
    by (metis last_snoc t.inject(1) to_e_list_1, metis to_e_list_1 append1_eq_conv, metis to_e_list_1)
  moreover
  hence "\<C> \<turnstile> [C v] : (ts _> ts@[t])"
    using assms(2) b_e_typing.const b_e_typing.weakening
    by fastforce
  ultimately
  show "\<S>\<bullet>\<C> \<turnstile> [$C v] : (ts _> ts')" "sec = Public"
    using e_typing_s_typing.intros(1)
    by fastforce+
qed
...
lemma types_preserved_e1:
  ...
  using assms
proof (induction arbitrary: tr tvs \<C> ts ts' arb_labs arb_return rule: reduce.induct)
...
next
  case (call s vs j i)
  obtain tr' ts'' tf1 tf2 where l_func_t: "trust_compat (trust_t \<C>) tr'"
                                          "length (func_t \<C>) > j"
                                          "ts = ts''@tf1"
                                          "ts' = ts''@tf2"
                                          "((func_t \<C>)!j) = (tr',(tf1 _> tf2))"
  ...
  have "j < length (func_t (s_inst \<S> ! i))"
    using l_func_t(2) call(4)
    by simp
  ultimately
  have "cl_typing \<S> (sfunc s i j) (tr',(tf1 _> tf2))"
    using store_typing_imp_func_agree[OF call(1)] l_func_t(5) call(4)
    by fastforce
  thus ?case
...
next
  ...
  show ?case
    ...
    by fastforce
next
  case (call_indirect_None c j s i vs tf)
  thus ?case
    using e_typing_s_typing.intros(4)
    by blast
next
  case (callcl_native cl i' j tfs es s t1s t2s ves vs n k m zs i)
  thus ?case
...
next
  ...
  then show ?case
    ...
    by fastforce
next
  case (load_Some s i k m sec off t v vs a)
  then show ?case
    using types_preserved_load(1) wasm_deserialise_type
    by blast
next
  case (load_None s i k off t vs a)
  then show ?case
    using e_typing_s_typing.intros(4)
    by blast
next
  case (load_packed_Some tp sx s i k off t v vs a)
  then show ?case
    using types_preserved_load(1) wasm_deserialise_type
    by blast
next
  case (load_packed_None s i k off tp vs t sx a)
  then show ?case
    using e_typing_s_typing.intros(4)
    by blast
next
  case (store_Some t v s' s i k off vs a)
  then show ?case
    using types_preserved_store
    by blast
next
  case (store_None t v s i k off vs a)
  then show ?case
    using e_typing_s_typing.intros(4)
    by blast
next
  case (store_packed_Some t v s' s i k off tp vs a)
  then show ?case
    using types_preserved_store
    by blast
next
  case (store_packed_None t v s i k off tp vs a)
  then show ?case
    using e_typing_s_typing.intros(4)
    by blast
next
  case (current_memory s i n c vs)
  then show ?case
    using types_preserved_current_memory
    by fastforce
next
  case (grow_memory s i n' c' c n s' vs)
  then show ?case
    using types_preserved_grow_memory
    by fastforce
next
  case (grow_memory_fail s vs c i)
  thus ?case
    using types_preserved_grow_memory
    by blast
next
...
next
  ...
  obtain \<C>' tls where es_def:"i < length (s_inst \<S>)"
                          "length tls = n"
                          "\<C>' = (s_inst \<S> ! i) \<lparr>trust_t := trust_t \<C>, local := local(s_inst \<S> ! i) @ map typeof vls, label := label (s_inst \<S> ! i), return := Some tls\<rparr>"
                          "\<S>\<bullet>\<C>' \<turnstile> es : ([] _> tls)"
                          "ts' = ts @ tls"
    using e_type_local[OF local(7)]
    by fastforce
  moreover
  obtain ts'' where "ts' = ts@ts''" "(\<S>\<bullet>(trust_t \<C>)\<bullet>(Some ts'') \<tturnstile>_i vls;es : ts'')"
  ...
  have "\<S>\<bullet>\<C>' \<turnstile> es' : ([] _> tls)" "map typeof vls = map typeof vs'"
    ...
    by fastforce+
  hence "\<S>\<bullet>(trust_t \<C>)\<bullet>(Some tls) \<tturnstile>_ i vs';es' : tls"
...
proof -
  ...
  obtain tvs \<C> where defs: "tvs = map typeof vs"
                           "\<C> = ((s_inst \<S>)!i)\<lparr>trust_t := tr, local := (local ((s_inst \<S>)!i) @ tvs), label := (label ((s_inst \<S>)!i)), return := None\<rparr>"
...
lemma const_list_no_progress:
  ...
  shows "\<not>\<lparr>s;vs;es\<rparr> a\<leadsto>_ i \<lparr>s';vs';es'\<rparr>"
proof -
  {
    assume "\<lparr>s;vs;es\<rparr> a\<leadsto>_ i \<lparr>s';vs';es'\<rparr>"
    hence "False"
      using assms
...
lemma progress_L0:
  assumes "\<lparr>s;vs;es\<rparr> a\<leadsto>_ i \<lparr>s';vs';es'\<rparr>"
          "const_list cs"
  shows "\<lparr>s;vs;cs@es@es_c\<rparr> a\<leadsto>_ i \<lparr>s';vs';cs@es'@es_c\<rparr>"
proof -
...
lemma progress_L0_left:
  assumes "\<lparr>s;vs;es\<rparr> a\<leadsto>_ i \<lparr>s';vs';es'\<rparr>"
          "const_list cs"
  shows "\<lparr>s;vs;cs@es\<rparr> a\<leadsto>_ i \<lparr>s';vs';cs@es'\<rparr>"
...
lemma progress_L0_trap:
  ...
  shows "\<exists>a. \<lparr>s;vs;cs@[Trap]@es\<rparr> a\<leadsto>_ i \<lparr>s;vs;[Trap]\<rparr>"
proof -
...
lemma const_of_i32:
  assumes "const_list cs"
          "\<S>\<bullet>\<C> \<turnstile> cs : ([] _> [(T_i32 sec)])"
...
proof -                    
  obtain v where "cs = [$C v]"
    ...
    by fastforce
  moreover
  hence "\<C> \<turnstile> [C v] : ([] _> [(T_i32 sec)])"
    using assms(2) unlift_b_e
    by fastforce
  hence "\<exists>c. v = ConstInt32 sec c"
  proof (induction "[C v]" "([] _> [(T_i32 sec)])" rule: b_e_typing.induct)
    case (const \<C>)
    then show ?case
      unfolding typeof_def
      by (cases v, auto)
...
lemma const_of_i64:
  assumes "const_list cs"
          "\<S>\<bullet>\<C> \<turnstile> cs : ([] _> [(T_i64 sec)])"
...
proof -                    
  obtain v where "cs = [$C v]"
    ...
    by fastforce
  moreover
  hence "\<C> \<turnstile> [C v] : ([] _> [(T_i64 sec)])"
    using assms(2) unlift_b_e
    by fastforce
  hence "\<exists>c. v = ConstInt64 sec c"
  proof (induction "[C v]" "([] _> [(T_i64 sec)])" rule: b_e_typing.induct)
    case (const \<C>)
    then show ?case
      unfolding typeof_def
      by (cases v, auto)
...
lemma progress_unop_testop_i:
  ...
  shows "\<exists>a s' vs' es'. \<lparr>s;vs;cs@([$e])\<rparr> a\<leadsto>_i \<lparr>s';vs';es'\<rparr>"
  using assms(2)
proof (cases t)
  case T_i32
  thus ?thesis
...
proof (cases t)
  case (T_i32 sec)
  hence cs_def:"\<exists>c1 c2. cs = [$C ConstInt32 sec c1,$C ConstInt32 sec c2]"
...
next
  case (T_i64 sec)
  hence cs_def:"\<exists>c1 c2. cs = [$C ConstInt64 sec c1,$C ConstInt64 sec c2]"
...
lemma progress_b_e:
  assumes "\<C> \<turnstile> b_es : (ts _> ts')"
          ...
          "option_projr (memory \<C>) = map_option (\<lambda>j. snd ((mem s)!j)) (smem_ind s i)"
  shows "\<exists>a s' vs' es'. \<lparr>s;vs;cs@($*b_es)\<rparr> a\<leadsto>_i \<lparr>s';vs';es'\<rparr>"
  using assms
...
next
  case (binop_i t \<C> uw)
  thus ?case
    using progress_binop_relop_i[OF binop_i(3,1)]
    by fastforce
next
  case (binop_f t \<C> ux)
  thus ?case
...
next
  ...
  obtain v where cs_def:"cs = [$ C v]" "typeof v = t2"
    using const_typeof const_of_const_list[OF _ convert(7)] e_type_const_list[OF convert(7,4)]
    by fastforce
  thus ?case
  proof (cases "cvt t1 sx v")
    case None
    thus ?thesis
      using reduce.intros(1)[OF reduce_simple.convert_None[OF _ None]] cs_def types_agree_imp_types_agree_insecure
      unfolding types_agree_def
      by fastforce
  next
    case (Some a)
    thus ?thesis
      using reduce.intros(1)[OF reduce_simple.convert_Some[OF _ Some]] cs_def types_agree_imp_types_agree_insecure
      unfolding types_agree_def
      by fastforce
  qed
next
  ...
  obtain v where cs_def:"cs = [$ C v]" "typeof v = t2"
    using const_typeof const_of_const_list[OF _ reinterpret(7)] e_type_const_list[OF reinterpret(7,4)]
    by fastforce
  thus ?case
    using reduce.intros(1)[OF reduce_simple.reinterpret[OF types_agree_imp_types_agree_insecure]]
    unfolding types_agree_def
    by fastforce
next
  case (classify t2 t1 \<C>)
  obtain v where cs_def:"cs = [$ C v]" "typeof v = t2"
    using const_typeof const_of_const_list[OF _ classify(7)] e_type_const_list[OF classify(7,4)]
    by fastforce
  thus ?case
    using reduce.intros(1)[OF reduce_simple.classify[OF types_agree_imp_types_agree_insecure]]
    unfolding types_agree_def
    by fastforce
next
  case (declassify \<C> t2 t1)
  obtain v where cs_def:"cs = [$ C v]" "typeof v = t2"
    using const_typeof const_of_const_list[OF _ declassify(8)] e_type_const_list[OF declassify(8,5)]
    by fastforce
  thus ?case
    using reduce.intros(1)[OF reduce_simple.declassify[OF types_agree_imp_types_agree_insecure]]
    unfolding types_agree_def
    by fastforce
next
...
next
  case (select sec t \<C>)
  obtain v1 v2 v3 where cs_def:"\<S>\<bullet>\<C> \<turnstile> [$ C v3] : ([] _> [(T_i32 sec)])"
                               "cs = [$C v1, $C v2, $ C v3]"
    using const_list_split_3[OF select(5,2)] select(5)
    unfolding const_list_def
    by (metis list_all_simps(1) e_type_const_unwrap)
  obtain c3 where c_def:"v3 = ConstInt32 sec c3"
    using cs_def select(5) const_of_i32[OF _ cs_def(1)]
    unfolding const_list_def
    by fastforce
  have "\<exists>a s' vs' es'. \<lparr>s;vs;[$C v1, $C v2, $ C ConstInt32 sec c3, $Select sec]\<rparr> a\<leadsto>_i \<lparr>s';vs';es'\<rparr>"
...
next
  ...
  obtain c1s c2s where cs_def:"\<S>\<bullet>\<C> \<turnstile> c1s : ([] _> tn)"
                              "\<S>\<bullet>\<C> \<turnstile> c2s : ([] _> [(T_i32 Public)])"
                              "const_list c1s"
                              "const_list c2s"
                              "cs = c1s @ c2s"
    using e_type_const_list_cons[OF if_wasm(9,6)] e_type_const_list
    by fastforce
  obtain c where c_def: "c2s = [$ C (ConstInt32 Public c)]"
    using const_of_i32 cs_def
    by fastforce
  have "\<exists>a s' vs' es'. \<lparr>s;vs;[$ C (ConstInt32 Public c), $ If tf es1 es2]\<rparr> a\<leadsto>_i \<lparr>s';vs';es'\<rparr>"
...
next
  ...
  obtain cs1 cs2 where cs_def:"\<S>\<bullet>\<C> \<turnstile> cs1 : ([] _> ts)"
                              "\<S>\<bullet>\<C> \<turnstile> cs2 : ([] _> [(T_i32 Public)])"
                              "const_list cs1"
                              "const_list cs2"
                              "cs = cs1 @ cs2"
    using e_type_const_list_cons[OF br_if(6,3)] e_type_const_list
    by fastforce
  obtain c where c_def:"cs2 = [$C ConstInt32 Public c]"
    using const_of_i32[OF cs_def(4,2)]
    by blast
  have "\<exists>a s' vs' es'. \<lparr>s;vs;cs2@($* [Br_if j])\<rparr> a\<leadsto>_i \<lparr>s';vs';es'\<rparr>"
  proof (cases "int_eq c 0")
    case True
    thus ?thesis
  ...
  qed
  thus ?case
    using cs_def(5) progress_L0[OF _ cs_def(3), of s vs "cs2 @ ($* [Br_if j])" _ _ _ _ _ "[]"]
    by fastforce
next
  ...
  obtain cs1 cs2 where cs_def:"\<S>\<bullet>\<C> \<turnstile> cs1 : ([]_> (t1s @ ts))"
                              ...
                              "const_list cs2"
                              "cs = cs1 @ cs2"
    using e_type_const_list_cons[OF br_table(5), of \<S> \<C> "(t1s @ ts)" "[(T_i32 Public)]"]
          e_type_const_list[of _ \<S> \<C> "t1s @ ts" "(t1s @ ts) @ [(T_i32 Public)]"]
          br_table(2,5)
    unfolding const_list_def
    by fastforce
  obtain c where c_def:"cs2 = [$C ConstInt32 Public c]"
    using const_of_i32[OF cs_def(4,2)]
    by blast
  have "\<exists>a s' vs' es'. \<lparr>s;vs;[$C ConstInt32 Public c, $Br_table is i']\<rparr> a\<leadsto>_i \<lparr>s';vs';es'\<rparr>"
...
next
  case (call \<C> tr j)
  show ?case
    using progress_L0[OF reduce.intros(2)[of s vs j i] call(7), of "[]"]
    by fastforce
next
  case (call_indirect \<C> tr j t1s t2s)
  obtain cs1 cs2 where cs_def:"\<S>\<bullet>\<C> \<turnstile> cs1 : ([]_> t1s)"
                              "\<S>\<bullet>\<C> \<turnstile> cs2 : ([] _> [(T_i32 Public)])"
                              "const_list cs1"
                              "const_list cs2"
                              "cs = cs1 @ cs2"
    using e_type_const_list_cons[OF call_indirect(8), of \<S> \<C> t1s "[(T_i32 Public)]"]
          e_type_const_list[of _ \<S> \<C> t1s "t1s @ [(T_i32 Public)]"]
          call_indirect(5)
    by fastforce
  obtain c where c_def:"cs2 = [$C ConstInt32 Public c]"
  ...
  hence "\<exists>a s' vs' es'. \<lparr>s;vs;[$C ConstInt32 Public c, $Call_indirect j]\<rparr> a\<leadsto>_i \<lparr>s';vs';es'\<rparr>"
  proof (cases)
    case 1
    thus ?thesis
...
next
  ...
  thus ?case
    ...
    by fastforce
next
  case (load \<C> n sec t a tp_sx off)
  obtain c where c_def: "cs = [$C ConstInt32 Public c]"
    using const_of_i32 load(4,7) e_type_const_unwrap
    unfolding const_list_def
    by fastforce
  obtain j where mem_some:"smem_ind s i = Some j"
    using load(1,11)
    unfolding smem_ind_def
    by (metis map_option_eq_Some option_projr_def)
  hence smem_sec:"snd (s.mem s ! j) = sec"
    using load(1,11)
    unfolding option_projr_def
    by simp
  have "\<exists>a' s' vs' es'. \<lparr>s;vs;[$C ConstInt32 Public c, $Load t tp_sx a off]\<rparr> a'\<leadsto>_i \<lparr>s';vs';es'\<rparr>"
  proof (cases tp_sx)
    case None
    note tp_none = None
    show ?thesis
    proof (cases "load (fst ((mem s)!j)) (nat_of_int c) off (t_length t)")
      case None
      show ?thesis
        using reduce.intros(13)[OF mem_some _ None, of sec vs] tp_none smem_sec load(2)
        by fastforce
    next
      case (Some a)
      show ?thesis
        using reduce.intros(12)[OF mem_some _ Some, of sec vs] tp_none smem_sec load(2)
        by fastforce
    qed
  next
    case (Some a)
    obtain tp sx where tp_some:"tp_sx = Some (tp, sx)"
      ...
      by fastforce
    show ?thesis
    proof (cases "load_packed sx (fst ((mem s)!j)) (nat_of_int c) off (tp_length tp) (t_length t)")
      case None
      show ?thesis
        using reduce.intros(15)[OF mem_some _ None, of sec vs Public] tp_some smem_sec load(2)
        by fastforce
    next
      case (Some a)
      show ?thesis
        using reduce.intros(14)[OF mem_some _ Some, of sec vs Public] tp_some smem_sec load(2)
        by fastforce
    qed
  qed
  then show ?case
...
next
  ...
  obtain cs' v where cs_def:"\<S>\<bullet>\<C> \<turnstile> [cs'] : ([] _> [(T_i32 Public)])"
                            "\<S>\<bullet>\<C> \<turnstile> [$ C v] : ([] _> [t])"
                            "cs = [cs',$ C v]"
    using const_list_split_2[OF store(7,4)] e_type_const_unwrap
    unfolding const_list_def
    by fastforce
  ...
  obtain c where c_def:"cs' = $C ConstInt32 Public c"
    ...
    unfolding const_list_def
    by fastforce
  have "\<exists>a' s' vs' es'. \<lparr>s;vs;[$C ConstInt32 Public c, $C v, $Store t tp a off]\<rparr> a'\<leadsto>_i \<lparr>s';vs';es'\<rparr>"
  proof (cases tp)
    case None
    note tp_none = None
    show ?thesis
    proof (cases "store (fst (s.mem s ! j)) (nat_of_int c) off (bits v) (t_length t)")
      case None
      show ?thesis
        using reduce.intros(17)[OF _ mem_some _ None, of sec vs Public] t_def tp_none smem_sec types_agree_imp_types_agree_insecure store(2)
        unfolding types_agree_def
        by fastforce
    next
      case (Some a)
      show ?thesis
        using reduce.intros(16)[OF _ mem_some _ Some, of sec vs Public] t_def tp_none smem_sec types_agree_imp_types_agree_insecure store(2)
        unfolding types_agree_def
        by fastforce
    qed
  next
    ...
    note tp_some = Some
    show ?thesis
    proof (cases "store_packed (fst (s.mem s ! j)) (nat_of_int c) off (bits v) (tp_length a)")
      case None
      show ?thesis
        using reduce.intros(19)[OF _ mem_some _ None, of t sec vs Public] t_def tp_some smem_sec types_agree_imp_types_agree_insecure store(2)
        unfolding types_agree_def
        by fastforce
    next
      case (Some a)
      show ?thesis
        using reduce.intros(18)[OF _ mem_some _ Some, of t sec vs Public] t_def tp_some smem_sec types_agree_imp_types_agree_insecure store(2)
        unfolding types_agree_def
        by fastforce
    qed
  qed
...
next
  ...
  proof (cases "s.mem s ! j")
    case (Pair a b)
    thus ?thesis
      using progress_L0[OF reduce.intros(20)[OF mem_some] current_memory(5), of _ _ _ vs "[]"]
      by fastforce
  qed
next
  case (grow_memory \<C> n sec)
  obtain c where c_def:"cs = [$C ConstInt32 Public c]"
  ...
  hence smem_sec:"snd (s.mem s ! j) = sec"
    ...
    by simp
  show ?case
    using reduce.intros(22)[OF mem_some, of _ sec] c_def smem_sec
...
next
  ...
  proof (cases)
    ...
    qed
    thus ?thesis
      using composition(2)[OF composition(5) _ _ composition(8) 1 composition(10,11,12)] progress_L0[of s vs "(cs @ ($* es))" _ i _ _ _ "[]" "$*[e]"]
      unfolding const_list_def
      by fastforce
  next
    case 2
...
next
  ...
  next
    ...
    thus False
      ...
      by simp
  qed
  hence "\<exists>a s' vs' es'. \<lparr>s;vs;cs2@($*es)\<rparr> a\<leadsto>_i \<lparr>s';vs';es'\<rparr>"
    using weakening(2)[OF cs_def(2) _ _ cs_def(5) weakening(7)] weakening(8-)
    by fastforce
  thus ?case
    using progress_L0[OF _ cs_def(4), of s vs "cs2 @ ($* es)" _ i _ _ _ "[]"] cs_def(3)
    by fastforce
qed

lemma progress_e:
  assumes "\<S>\<bullet>tr\<bullet>None \<tturnstile>_i vs;cs_es : ts'"
          ...
          "store_typing s \<S>"
  shows "\<exists>a s' vs' es'. \<lparr>s;vs;cs_es\<rparr> a\<leadsto>_i \<lparr>s';vs';es'\<rparr>"
proof -
  fix \<C> cs es ts_c
  have prems1:
      "\<S>\<bullet>\<C> \<turnstile> es : (ts_c _> ts') \<Longrightarrow>
       ...
       option_projr (memory \<C>) = map_option (\<lambda>j. snd ((mem s)!j)) (smem_ind s i)  \<Longrightarrow>
         \<exists>a s' vs' cs_es'. \<lparr>s;vs;cs_es\<rparr> a\<leadsto>_i \<lparr>s';vs';cs_es'\<rparr>"
   and prems2:
      "\<S>\<bullet>tr\<bullet>None \<tturnstile>_i vs;cs_es : ts' \<Longrightarrow>
       (\<And> k lholed. \<not>(Lfilled k lholed [$Return] cs_es)) \<Longrightarrow>
       (\<And> i k lholed. (Lfilled k lholed [$Br (i)] cs_es) \<Longrightarrow> i < k) \<Longrightarrow>
       cs_es \<noteq> [Trap] \<Longrightarrow>
       \<not> const_list (cs_es) \<Longrightarrow>
      ...
      next
        ...
        thus ?thesis
          using 2(3)[OF _ _ 2(7,8) _ _ _ _ 2(13,14,15)] preds 2(6,16)
                progress_L0[of s vs "(cs @ es)" _ _ _ _ _ "[]" "[e]"]
          unfolding const_list_def
          by (metis append.assoc append_Nil list.pred_inject(1))
      qed
    qed
  ...
  case (5 \<S> \<C> ts j vls es n)
    ...
    proof (cases)
      case 1
      obtain s' vs'' a a' where temp1:"\<lparr>s;vls;es\<rparr> a'\<leadsto>_ j \<lparr>s';vs'';a\<rparr>"
    ...
    next
      ...
      then obtain lholed' vs' \<C>' where lholed'_def:"(Lfilled k lholed' (vs'@[$Return]) es)"
        ...
        by fastforce
      hence temp1:"\<exists>a. \<lparr>[Local n j vls es]\<rparr> a\<leadsto> \<lparr>vs'\<rparr>"
    ...
    next
      ...
      then consider (1) "const_list es" | (2) "es = [Trap]"
        by blast
      hence temp1:"\<exists>a. \<lparr>s;vs;[Local n j vls es]\<rparr> a\<leadsto>_ i \<lparr>s;vs;es\<rparr>"
      proof (cases)
        case 1
    ...
    next
      ...
      then obtain k' lholed' i' where temp1:"Lfilled k' lholed' [$Br (k'+i')] es"
        using le_Suc_ex
        by blast
      obtain \<C>' where c_def:"\<C>' = ((s_inst \<S>)!j)\<lparr>trust_t := trust_t \<C>, local := (local ((s_inst \<S>)!j)) @ (map typeof vls), return := Some ts\<rparr>"
        by blast
  ...
  next
    ...
    obtain ts'' where ts''_def:"\<S>\<bullet>\<C> \<turnstile> cs : ([] _> ts'')" "\<S>\<bullet>\<C> \<turnstile> [Callcl cl] : (ts'' _> ts')"
      ...
      by fastforce
    obtain ts_c tr' t1s t2s where cl_def:"(ts'' = ts_c @ t1s)"
                                         "(ts' = ts_c @ t2s)"
                                         "cl_type cl = (tr',(t1s _> t2s))"
    ...
    obtain vs1 vs2 where vs_def:"\<S>\<bullet>\<C> \<turnstile> vs1 : ([] _> ts_c)"
                                ...
                                "const_list vs1"
                                "const_list vs2"
      using e_type_const_list_cons[OF 6(5)] ts''_def(1) cl_def(1)
      by fastforce
    ...
    proof (cases cl)
      ...
      hence func_native_def:"cl = Func_native x11 (tr',(t1s _> t2s)) x13 x14"
        using cl_def(3)
        unfolding cl_type_def
        by simp
      have "\<exists>a a'. \<lparr>s;vs;vs2 @ [Callcl cl]\<rparr> a'\<leadsto>_ i \<lparr>s;vs;a\<rparr>"
        using reduce.intros(5)[OF func_native_def] e_type_const_conv_vs[OF vs_def(5)] l
        unfolding n_zeros_def
        by fastforce
      thus ?thesis
        using progress_L0 vs_def(3,4) 6(4)
        by fastforce
    next
      case (Func_host x21 x22)
      hence func_host_def:"cl = Func_host (tr',(t1s _> t2s)) x22"
        using cl_def(3)
        unfolding cl_type_def
        by simp
      obtain vcs where vcs_def:"vs2 = $$* vcs"
        ...
        by blast
      fix hs
      have "\<exists>s' a a'. \<lparr>s;vs;vs2 @ [Callcl cl]\<rparr> a'\<leadsto>_ i \<lparr>s';vs;a\<rparr>"
      ...
      qed
      thus ?thesis
        using vs_def(3,4) 6(4) progress_L0
        by fastforce
    qed
  next
    ...
    proof (cases)
      ...
      have temp2:"\<S>\<bullet>\<C>\<lparr>label := [ts] @ label \<C>\<rparr> \<turnstile> [] : ([] _> [])"
        ...
        by fastforce
      have "\<exists>s' vs' a a'. \<lparr>s;vs;es\<rparr> a'\<leadsto>_ i \<lparr>s';vs';a\<rparr>"
        using 7(5)[OF 7(2), of "[]" "[]", OF temp1 temp2 1(1) _ 1(3,4) 7(14,15)]
              1(2) 7(16,17)
        unfolding const_list_def
        by fastforce
      then obtain s' vs' a where red_def:"\<exists>a'. \<lparr>s;vs;es\<rparr> a'\<leadsto>_ i \<lparr>s';vs';a\<rparr>"
        by blast
    ...
    next
      case 3
      hence temp1:"\<exists>a. \<lparr>s;vs;[Label n e0s es]\<rparr> a\<leadsto>_ i \<lparr>s;vs;es\<rparr>"
    ...
    next
      ...
      then obtain lholed' vs' \<C>' where lholed'_def:"(Lfilled k lholed' (vs'@[$Br (k)]) es)"
        ...
        by fastforce
      have "\<exists>es' a. \<lparr>[Label n e0s es]\<rparr> a\<leadsto> \<lparr>vs'@e0s\<rparr>"
        using reduce_simple.br[OF lholed'_def(3) _ lholed'_def(1)] 7(3)
              e_type_const_list[OF lholed'_def(3,2)]
        by fastforce
      hence "\<exists>es' a. \<lparr>s;vs;[Label n e0s es]\<rparr> a\<leadsto>_ i \<lparr>s;vs;es'\<rparr>"
  ...
  next
    ...
    have "length (local \<C>) = length vs"
      ...
      by fastforce
    moreover have "option_projr (memory \<C>) = map_option (\<lambda>j. snd (s.mem s ! j)) (smem_ind s i)"
      using store_typing_imp_mem_agree_inst[OF 8(11,1)] 8(3)
      by simp
...
lemma progress_e1:
  assumes "\<S>\<bullet>tr\<bullet>None \<tturnstile>_i vs;es : ts"
...
proof -
  {
    ...
    obtain \<C> where c_def:"i < length (s_inst \<S>)"
                   "\<C> = ((s_inst \<S>)!i)\<lparr>trust_t := tr, local := (local ((s_inst \<S>)!i)) @ (map typeof vs), return := None\<rparr>"
                   "(\<S>\<bullet>\<C> \<turnstile> es : ([] _> ts))"
