...
lemma is_float_public_t:
  assumes "is_float_t t"
  shows "is_public_t t"
  using assms
  unfolding is_float_t_def t_sec_def
  by (cases t) auto

lemma is_secret_int_t:
  assumes "is_secret_t t"
  shows "is_int_t t"
  using assms
  unfolding is_int_t_def t_sec_def
  by (cases t) auto

lemma typeof_i32:
  assumes "typeof v = (T_i32 sec)"
...
lemma is_int_t_classify_t:
  assumes "is_int_t t"
          "is_public_t t"
  shows "is_int_t (classify_t t)"
  using assms
  unfolding is_int_t_def classify_t_def t_sec_def
  by (cases t) auto

lemma classify_t_classify_typeof:
  assumes "types_agree_insecure t v"
  shows "(classify_t t) = typeof (classify v)"
  using assms
  by (cases t; cases v) (auto simp add: types_agree_insecure_def
                                        classify_def
                                        typeof_def
                                        classify_t_def
                                        t_length_def
...
lemma declassify_t_declassify_typeof:
  assumes "types_agree_insecure t v"
  shows "(declassify_t t) = typeof (declassify v)"
  using assms
  by (cases t; cases v) (auto simp add: types_agree_insecure_def
                                        declassify_def
                                        typeof_def
...
lemma b_e_type_load:
  ...
  shows "\<exists>ts'' sec n. ts = ts''@[(T_i32 Public)] \<and> ts' = ts''@[t] \<and> (memory \<C>) = Some (n,sec) \<and> t_sec t = sec"
...
lemma b_e_type_store:
  assumes "\<C> \<turnstile> [e] : (ts _> ts')"
          "e = Store t tp a off"
    shows "ts = ts'@[(T_i32 Public), t]"
          "\<exists>sec n. (memory \<C>) = Some (n,sec) \<and> t_sec t = sec"
...
lemma b_e_type_current_memory:
  assumes "\<C> \<turnstile> [e] : (ts _> ts')"
          "e = Current_memory"
  shows "\<exists>sec n. ts' = ts @ [(T_i32 Public)] \<and> (memory \<C>) = Some (n,sec)"
  using assms
  by (induction "[e]" "(ts _> ts')" arbitrary: ts ts' rule: b_e_typing.induct, auto)
...
lemma b_e_type_grow_memory:
  assumes "\<C> \<turnstile> [e] : (ts _> ts')"
          "e = Grow_memory"
  shows "\<exists>ts''. ts = ts''@[(T_i32 Public)] \<and> ts = ts' \<and> (\<exists>n. (memory \<C>) = Some n)"
  using assms
  by (induction "[e]" "(ts _> ts')" arbitrary: ts ts' rule: b_e_typing.induct) auto
...
definition arity_2_result :: "b_e \<Rightarrow> t" where
  "arity_2_result op2 = (case op2 of
                         ...
                         | Relop_i t _ \<Rightarrow> (T_i32 (t_sec t))
                         | Relop_f t _ \<Rightarrow> (T_i32 (t_sec t)))"
...
definition arity_1_result :: "b_e \<Rightarrow> t" where
  "arity_1_result op1 = (case op1 of
                         ...
                         | Testop t _ \<Rightarrow> (T_i32 (t_sec t))
                         | Cvtop t1 Convert _ _ \<Rightarrow> t1
                         | Cvtop t1 Reinterpret _ _ \<Rightarrow> t1
                         | Cvtop _ Classify t2 _ \<Rightarrow> classify_t t2
                         | Cvtop _ Declassify t2 _ \<Rightarrow> declassify_t t2)"
...
lemma b_e_type_select:
  assumes "\<C> \<turnstile> [e] : (ts _> ts')"
          "e = Select sec"
  shows "\<exists>ts'' t. ts = ts''@[t,t,(T_i32 sec)] \<and> ts' = ts''@[t] \<and> (sec = Secret \<longrightarrow> is_secret_t t)"
  using assms
  by (induction "[e]" "(ts _> ts')" arbitrary: ts ts' rule: b_e_typing.induct, auto)
...
lemma b_e_type_call:
  ...
  shows  "i < length (func_t \<C>)"
         "\<exists>tr ts'' tf1 tf2. trust_compat (trust_t \<C>) tr \<and> ts = ts''@tf1 \<and> ts' = ts''@tf2 \<and> (func_t \<C>)!i = (tr,(tf1 _> tf2))"
  using assms
  by (induction "[e]" "(ts _> ts')" arbitrary: ts ts' rule: b_e_typing.induct, auto)
...
lemma b_e_type_call_indirect:
  ...
  shows "i < length (types_t \<C>)"
        "\<exists>tr ts'' tf1 tf2. trust_compat (trust_t \<C>) tr \<and> ts = ts''@tf1@[(T_i32 Public)] \<and> ts' = ts''@tf2 \<and> (types_t \<C>)!i = (tr,(tf1 _> tf2))"
  using assms
  by (induction "[e]" "(ts _> ts')" arbitrary: ts ts' rule: b_e_typing.induct, auto)
...
lemma b_e_type_if:
  assumes "\<C> \<turnstile> [e] : (ts _> ts')"
          "e = If tf es1 es2"
  shows "\<exists>ts'' tfn tfm. tf = (tfn _> tfm) \<and> (ts = ts''@tfn @ [(T_i32 Public)]) \<and> (ts' = ts''@tfm) \<and>
...
lemma b_e_type_br_if:
  assumes "\<C> \<turnstile> [e] : (ts _> ts')"
          "e = Br_if i"
        shows "i < length(label \<C>)"
              "\<exists>ts_c ts''. ts = ts_c @ ts'' @ [(T_i32 Public)] \<and> ts' = ts_c @ ts'' \<and> (label \<C>)!i = ts''"
  using assms
  by (induction "[e]" "(ts _> ts')" arbitrary: ts ts' rule: b_e_typing.induct, auto)
...
lemma b_e_type_br_table:
  assumes "\<C> \<turnstile> [e] : (ts _> ts')"
          "e = Br_table is i"
  shows "\<exists>ts_c ts''. list_all (\<lambda>i. i < length(label \<C>) \<and> (label \<C>)!i = ts'') (is@[i]) \<and> ts = ts_c @ ts''@[(T_i32 Public)]"
  using assms
  by (induction "[e]" "(ts _> ts')" arbitrary: ts ts' rule: b_e_typing.induct, fastforce+)
...
lemma e_type_callcl_native:
  assumes "\<S>\<bullet>\<C> \<turnstile> [Callcl cl] : (t1s' _> t2s')"
          "cl = Func_native i (tr,tf) ts es"
  shows "\<exists>t1s t2s ts_c. trust_compat (trust_t \<C>) tr
                         \<and>(t1s' = ts_c @ t1s)
                         \<and> (t2s' = ts_c @ t2s)
                         \<and> tf = (t1s _> t2s)
                         \<and> i < length (s_inst \<S>)
                         \<and> (((s_inst \<S>)!i)\<lparr>trust_t := tr, local := (local ((s_inst \<S>)!i)) @ t1s @ ts, label := ([t2s] @ (label ((s_inst \<S>)!i))), return := Some t2s\<rparr>  \<turnstile> es : ([] _> t2s))"
  using assms
...
lemma e_type_callcl_host:
  assumes "\<S>\<bullet>\<C> \<turnstile> [Callcl cl] : (t1s' _> t2s')"
          "cl = Func_host tf f"
  shows "\<exists>tr t1s t2s ts_c. trust_compat (trust_t \<C>) tr
                           \<and> (t1s' = ts_c @ t1s)
                           \<and> (t2s' = ts_c @ t2s)
                           \<and> tf = (tr,(t1s _> t2s))"
  using assms
...
lemma s_type_unfold:
  assumes "\<S>\<bullet>tr\<bullet>rs \<tturnstile>_i vs;es : ts"
  shows "i < length (s_inst \<S>)"
        "(rs = Some ts) \<or> rs = None"
        "(\<S>\<bullet>((s_inst \<S>)!i)\<lparr>trust_t := tr, local := (local ((s_inst \<S>)!i)) @ (map typeof vs), return := rs\<rparr> \<turnstile> es : ([] _> ts))"
  using assms
...
lemma e_type_local:
  ...
  shows "\<exists>tls. i < length (s_inst \<S>)
               \<and> length tls = n
               \<and> (\<S>\<bullet>((s_inst \<S>)!i)\<lparr>trust_t := (trust_t \<C>), local := (local ((s_inst \<S>)!i)) @ (map typeof vs), return := Some tls\<rparr> \<turnstile> es : ([] _> tls))
               \<and> ts' = ts @ tls"
  using assms
...
lemma e_type_local_shallow:
  assumes "\<S>\<bullet>\<C> \<turnstile> [Local n i vs es] : (ts _> ts')"
  shows "\<exists>tls. length tls = n \<and> ts' = ts@tls \<and> (\<S>\<bullet>(trust_t \<C>)\<bullet>(Some tls) \<tturnstile>_i vs;es : tls)"
  using assms
proof (induction "\<S>" "\<C>" "[Local n i vs es]" "(ts _> ts')" arbitrary: ts ts')
  case (1 \<C> b_es \<S>)
  thus ?case
...
lemma store_typing_imp_mem_agree_inst:
  assumes "store_typing s \<S>"
          "i < length (s_inst \<S>)"
  shows "option_projr (memory ((s_inst \<S>)!i)) = map_option (\<lambda>j. snd ((mem s)!j)) (smem_ind s i)"
proof (cases "smem_ind s i")
  case None
  thus ?thesis
    using assms store_typing_imp_mem_agree_None
    unfolding option_projr_def
    by fastforce
next
  case (Some j)
  show ?thesis
    using store_typing_imp_mem_agree_Some[OF assms Some] Some
    unfolding option_projr_def mem_agree_def
    by fastforce
qed

lemma store_preserved_mem:
  assumes "store_typing s \<S>"
          "s' = s\<lparr>s.mem := (s.mem s)[i := (mem',sec)]\<rparr>"
          "mem_size mem' \<ge> mem_size orig_mem"
          "((s.mem s)!i) = (orig_mem, sec)"
...
proof -
  ...
  proof -
    ...
    have initial_mem:"list_all2 (\<lambda> (bs,sec) (m,sec'). m \<le> mem_size bs \<and> sec = sec') bss ms"
      using assms(1) s_S_defs
      unfolding store_typing.simps mem_agree_def
      by blast
    have "\<And>n. n < length bss \<Longrightarrow> (\<lambda> (bs,sec) (m,sec'). m \<le> mem_size bs \<and> sec = sec') (bss'!n) (ms!n)"
    proof -
      fix n
      assume local_assms:"n < length bss"
      obtain \<C>_m where cmdef:"\<C>_m = \<C>s ! n"
        by blast
      hence "(\<lambda> (bs,sec) (m,sec'). m \<le> mem_size bs \<and> sec = sec') (bss!n) (ms!n)"
        using initial_mem local_assms
        unfolding list_all2_conv_all_nth
        by simp
      thus "(\<lambda> (bs,sec) (m,sec'). m \<le> mem_size bs \<and> sec = sec') (bss'!n) (ms!n)"
...
lemma b_e_typing_trust_compat:
  assumes "\<C> \<turnstile> es : tf"
          "trust_compat tr (trust_t \<C>)"
  shows "\<C>\<lparr>trust_t := tr\<rparr> \<turnstile> es : tf"
  using assms
proof (induction rule: b_e_typing.induct)
  case (block tf tn tm \<C> es)
  have "\<C>\<lparr>label := [tm] @ label \<C>, trust_t := tr\<rparr> \<turnstile> es : (tn _> tm)"
    using block(3,4)
    by simp
  moreover
  have "\<C>\<lparr>label := [tm] @ label \<C>, trust_t := tr\<rparr> = \<C>\<lparr>trust_t := tr\<rparr>\<lparr>label := [tm] @ label \<C>\<rparr>"
    by simp
  ultimately
  have "\<C>\<lparr>trust_t := tr\<rparr>\<lparr>label := [tm] @ label \<C>\<rparr> \<turnstile> es : (tn _> tm)"
    by metis
  thus ?case
    using b_e_typing.block[OF block(1)]
    by simp
next
  case (loop tf tn tm \<C> es)
  have "\<C>\<lparr>label := [tn] @ label \<C>, trust_t := tr\<rparr> \<turnstile> es : (tn _> tm)"
    using loop(3,4)
    by simp
  moreover
  have "\<C>\<lparr>label := [tn] @ label \<C>, trust_t := tr\<rparr> = \<C>\<lparr>trust_t := tr\<rparr>\<lparr>label := [tn] @ label \<C>\<rparr>"
    by simp
  ultimately
  have "\<C>\<lparr>trust_t := tr\<rparr>\<lparr>label := [tn] @ label \<C>\<rparr> \<turnstile> es : (tn _> tm)"
    by metis
  thus ?case
    using b_e_typing.loop[OF loop(1)]
    by simp
next
  case (if_wasm tf tn tm \<C> es1 es2)
  have "\<C>\<lparr>label := [tm] @ label \<C>, trust_t := tr\<rparr> \<turnstile> es1 : (tn _> tm)"
       "\<C>\<lparr>label := [tm] @ label \<C>, trust_t := tr\<rparr> \<turnstile> es2 : (tn _> tm)"
    using if_wasm(4,5,6)
    by simp_all
  moreover
  have "\<C>\<lparr>label := [tm] @ label \<C>, trust_t := tr\<rparr> = \<C>\<lparr>trust_t := tr\<rparr>\<lparr>label := [tm] @ label \<C>\<rparr>"
    by simp
  ultimately
  have "\<C>\<lparr>trust_t := tr\<rparr>\<lparr>label := [tm] @ label \<C>\<rparr> \<turnstile> es1 : (tn _> tm)"
       "\<C>\<lparr>trust_t := tr\<rparr>\<lparr>label := [tm] @ label \<C>\<rparr> \<turnstile> es2 : (tn _> tm)"
    by metis+
  thus ?case
    using b_e_typing.if_wasm[OF if_wasm(1)]
    by simp
qed (auto simp add: b_e_typing.intros trust_compat_def)

lemma e_typing_s_typing_trust_compat:
  "\<S>\<bullet>\<C> \<turnstile> es : tf \<Longrightarrow> trust_compat tr (trust_t \<C>) \<Longrightarrow> \<S>\<bullet>\<C>\<lparr>trust_t := tr\<rparr> \<turnstile> es : tf"
  "\<S>\<bullet>tr'\<bullet>r \<tturnstile>_i vs;es : ts \<Longrightarrow> trust_compat tr tr' \<Longrightarrow> \<S>\<bullet>tr\<bullet>r \<tturnstile>_i vs;es : ts"
proof (induction rule: e_typing_s_typing.inducts)
  case (1 \<C> b_es tf \<S>)
  thus ?case
    using b_e_typing_trust_compat e_typing_s_typing.intros(1)
    by simp
next
  case (6 \<C> tr \<S> cl tf)
  thus ?case
...
next
  ...
  have "\<S>\<bullet>\<C>\<lparr>trust_t := tr\<rparr> \<turnstile> e0s : (ts _> t2s)"
       "\<S>\<bullet>\<C> \<lparr>label := [ts] @ label \<C>, trust_t := tr\<rparr> \<turnstile> es : ([] _> t2s)"
    using 7(4,5,6)
    by simp_all
  moreover
  have "\<C> \<lparr>label := [ts] @ label \<C>, trust_t := tr\<rparr> = \<C>\<lparr>trust_t := tr\<rparr>\<lparr>label := [ts] @ label (\<C>\<lparr>trust_t := tr\<rparr>)\<rparr>"
    by simp
  ultimately
  have "\<S>\<bullet>\<C>\<lparr>trust_t := tr\<rparr> \<turnstile> e0s : (ts _> t2s)"
       "\<S>\<bullet>\<C>\<lparr>trust_t := tr\<rparr>\<lparr>label := [ts] @ label (\<C>\<lparr>trust_t := tr\<rparr>)\<rparr> \<turnstile> es : ([] _> t2s)"
  by metis+
  thus ?case
    using e_typing_s_typing.intros(7) 7(3)
    by fastforce
next
  case (8 i \<S> tvs vs \<C> rs tr' es ts)
  have "trust_compat tr (trust_t \<C>)"
    using 8(3,7)
    by simp
  hence "\<S>\<bullet>\<C>\<lparr>trust_t := tr\<rparr> \<turnstile> es : ([] _> ts)"
    using 8(6)
    by simp
  moreover
  have "\<C>\<lparr>trust_t := tr\<rparr> = (s_inst \<S> ! i) \<lparr>trust_t := tr, local := local (s_inst \<S> ! i) @ tvs, return := rs\<rparr>"
    using 8(3)
    by simp
  ultimately
  show ?case
    using e_typing_s_typing.intros(8)[OF 8(1,2) _ _ 8(5)]
    by fastforce
qed (simp_all add: e_typing_s_typing.intros)
