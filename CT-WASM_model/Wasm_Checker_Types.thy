...
fun can_secret_ct :: "ct  \<Rightarrow> bool" where
  "can_secret_ct (TSome t) = is_secret_t t" 
| "can_secret_ct _ = True"

fun sec_ct :: "sec \<Rightarrow> ct" where
  "sec_ct Public = TAny"
| "sec_ct Private = TSecret"

fun ct_eq :: "ct \<Rightarrow> ct \<Rightarrow> bool" where
  "ct_eq (TSome t) (TSome t') = (t = t')"
| "ct_eq TSecret ct = can_secret_ct ct"
| "ct_eq ct TSecret = can_secret_ct ct"
| "ct_eq TAny _ = True"
| "ct_eq _ TAny = True"
...
lemma ct_list_eq_refl: "ct_list_eq xs xs"
  unfolding ct_list_eq_def
  by (metis can_secret_ct.simps(3) ct.simps(1) ct_eq.elims(3) list.rel_refl)
...
next
  ...
  thus ?case
    unfolding ct_list_eq_def to_ct_list_def
    apply (cases a)
    apply (metis ct_eq.simps(6) list.simps(9) list_all2_Cons1)
    apply (metis (full_types) can_secret_ct.simps(1) ct_eq.simps(4) ct_eq_commute exists_secret list.rel_inject(2) list.simps(9))
    apply (metis (full_types) ct_list_eq_def ct_list_eq_refl list.rel_inject(2) list.simps(9))
    done
qed
...
lemma ct_suffix_cons_ct_list_single:
  assumes "ct_suffix (xs@[y]) (to_ct_list zs)"
  shows "\<exists>as b. zs = as@[b] \<and> ct_eq y (TSome b) \<and> ct_suffix xs (to_ct_list as)"
  using assms ct_suffix_cons_ct_list[of xs "[y]" zs]
  unfolding ct_list_eq_def to_ct_list_def
  by (simp add: list_all2_map2)
     (metis (no_types, lifting) list_all2_Cons1 list_all2_Nil)
...
proof -
  ...
  hence "ct_list_eq (a#bs) (TAny#xs)"
    using ys_def
    by (metis can_secret_ct.elims(3) ct.distinct(1,3) ct_eq.elims(3) ct_list_eq_def list.rel_inject(2))
  thus ?thesis
...
lemma ct_suffix_singleton_any: "ct_suffix [TAny] [t]"
  ...
  by (simp add: ct_suffix_extend_any1)
...
fun ens_sec_ct :: "sec \<Rightarrow> ct \<Rightarrow> ct" where
  "ens_sec_ct Secret TAny = TSecret"
| "ens_sec_ct _ ct = ct"

fun select_return_top :: "sec \<Rightarrow> ct list \<Rightarrow> ct \<Rightarrow> ct \<Rightarrow> checker_type" where
  "select_return_top sec ts ct1 TAny = (if (sec = Secret \<longrightarrow> can_secret_ct ct1)
                                          then TopType ((take (length ts - 3) ts) @ [ens_sec_ct sec ct1])
                                          else Bot)"
| "select_return_top sec ts TAny ct2 = (if (sec = Secret \<longrightarrow> can_secret_ct ct2)
                                          then TopType ((take (length ts - 3) ts) @ [ens_sec_ct sec ct2])
                                          else Bot)"
| "select_return_top sec ts ct1 TSecret = (if (can_secret_ct ct1)
                                          then TopType ((take (length ts - 3) ts) @ [ens_sec_ct sec ct1])
                                          else Bot)"
| "select_return_top sec ts TSecret ct2 = (if (can_secret_ct ct2)
                                          then TopType ((take (length ts - 3) ts) @ [ct2])
                                          else Bot)"
| "select_return_top sec ts (TSome t1) (TSome t2) = (if (t1 = t2 \<and> (sec = Secret \<longrightarrow> is_secret_t t1))
                                                   then (TopType ((take (length ts - 3) ts) @ [TSome t1]))
                                                   else Bot)"

lemma select_return_top_ens_sec_ct:
  assumes "select_return_top sec ts ct1 ct2 = ct'"
  shows "ct' = Bot \<or> ct' = TopType ((take (length ts - 3) ts) @ [ens_sec_ct sec ct1]) \<or> ct' = TopType ((take (length ts - 3) ts) @ [ens_sec_ct sec ct2])"
  using assms
  by (cases ct1; cases ct2)  (auto split: if_splits)

lemma select_return_top_ens_sec_ct_not_bot:
  assumes "select_return_top sec ts ct1 ct2 = ct'"
          "ct' \<noteq> Bot"
  shows "ct' = TopType ((take (length ts - 3) ts) @ [ens_sec_ct sec ct1]) \<or> ct' = TopType ((take (length ts - 3) ts) @ [ens_sec_ct sec ct2])"
  using assms select_return_top_ens_sec_ct
  by fastforce

fun type_update_select :: "sec \<Rightarrow> checker_type \<Rightarrow> checker_type" where
  "type_update_select sec (Type ts) = (if (length ts \<ge> 3 \<and> (ts!(length ts-2)) = (ts!(length ts-3)) \<and> (sec = Secret \<longrightarrow> is_secret_t (ts!(length ts-2))))
                                    then consume (Type ts) [TAny, TSome (T_i32 sec)]
                                    else Bot)"
| "type_update_select sec (TopType ts) = (case length ts of
                                            0 \<Rightarrow> TopType [sec_ct sec]
                                          | Suc 0 \<Rightarrow> type_update (TopType ts) [TSome (T_i32 sec)] (TopType [sec_ct sec])
                                          | Suc (Suc 0) \<Rightarrow> type_update (TopType ts) [sec_ct sec, TSome (T_i32 sec)] (TopType [ens_sec_ct sec (ts!(length ts-2))])
                                          | _  \<Rightarrow> type_update (TopType ts) [sec_ct sec, sec_ct sec, TSome (T_i32 sec)]
                                                          (select_return_top sec ts (ts!(length ts-2)) (ts!(length ts-3))))"
| "type_update_select _ _ = Bot"
...
lemma select_return_top_sec:
  assumes "select_return_top sec ts ct1 ct2 = ts'"
          "ts' \<noteq> Bot"
  shows "(sec = Secret \<longrightarrow> can_secret_ct ct1) \<and> (sec = Secret \<longrightarrow> can_secret_ct ct2)"
  using assms
  by (cases ct1; cases ct2) (auto split: if_split_asm)

lemma produce_not_bot:
  assumes "produce a b = c"
          "c \<noteq> Bot"
  shows "a \<noteq> Bot" "b \<noteq> Bot"
  using assms
...
lemma type_update_select_length0:
  assumes "type_update_select sec (TopType cts) = tm"
          "length cts = 0"
          "tm \<noteq> Bot"
  shows "tm = TopType [sec_ct sec]"
  using assms
  by simp
...
lemma type_update_select_length1:
  assumes "type_update_select sec (TopType cts) = tm"
          "length cts = 1"
          "tm \<noteq> Bot"
  shows "ct_list_eq cts [TSome (T_i32 sec)]"
        "tm = TopType [sec_ct sec]"
proof -
  have 1:"type_update (TopType cts) [TSome (T_i32 sec)] (TopType [sec_ct sec]) = tm"
    using assms(1,2)
    by simp
  hence "ct_suffix cts [TSome (T_i32 sec)] \<or> ct_suffix [TSome (T_i32 sec)] cts"
    using assms(3)
    by (metis consume.simps(2) produce.simps(7) type_update.simps)
  thus "ct_list_eq cts [TSome (T_i32 sec)]"
...
lemma type_update_select_length2:
  assumes "type_update_select sec (TopType cts) = tm"
          "length cts = 2"
          "tm \<noteq> Bot"
  shows "\<exists>t1 t2. cts = [t1, t2] \<and> ct_eq t2 (TSome (T_i32 sec)) \<and> ct_eq t1 (sec_ct sec) \<and> tm = TopType [ens_sec_ct sec t1]"
proof -
  ...
  moreover
  hence "type_update (TopType [x,y]) [sec_ct sec, TSome (T_i32 sec)] (TopType [ens_sec_ct sec x])  = tm"
    using assms cts_def
    by (simp split: if_splits)
  moreover
  hence ct_disj_is:"ct_suffix [x,y] [sec_ct sec, TSome (T_i32 sec)] \<or> ct_suffix [sec_ct sec, TSome (T_i32 sec)] [x,y]"
    using assms(3)
    by (metis consume.simps(2) produce.simps(7) type_update.simps)
  have "ct_list_eq [sec_ct sec, TSome (T_i32 sec)] ([x,y])"
  proof (cases "ct_suffix [x,y] [sec_ct sec, TSome (T_i32 sec)]")
    case True
    thus ?thesis
      by (metis append_Nil ct_suffixI ct_suffix_cons2 length_Cons)
  next
    case False
    thus ?thesis
      using ct_disj_is
      by (metis append_Nil ct_suffix_cons2 length_Cons)
  qed
  ultimately
  show ?thesis
    by (metis assms(3) consume_top_leq ct_eq_commute ct_list_eq_def ct_suffix_length ct_suffix_refl length_Cons list.simps(11) produce.simps(4,7) type_update.simps)
qed

lemma type_update_select_length3:
  assumes "type_update_select sec (TopType cts) = (TopType ctm)"
          "length cts \<ge> 3"
  shows "\<exists>cts' ct1 ct2 ct3. cts = cts'@[ct1, ct2, ct3] \<and> ct_eq ct3 (TSome (T_i32 sec))
         \<and> ct_eq ct1 (sec_ct sec) \<and> ct_eq ct2 (sec_ct sec)"
proof -
  ...
  obtain nat where "length cts = Suc (Suc (Suc nat))"
    using assms(2)
    by (simp add: cts_def)
  hence "(type_update (TopType cts) [sec_ct sec, sec_ct sec, TSome (T_i32 sec)]
            (select_return_top sec cts (cts!(length cts-2)) (cts!(length cts-3)))) = TopType ctm"
    using assms
    by simp
  then obtain c_mid where "consume (TopType cts) [sec_ct sec, sec_ct sec, TSome (T_i32 sec)] = TopType c_mid"
    by (metis consume.simps(2) produce.simps(6) type_update.simps)
  hence ct_suff:"ct_suffix [sec_ct sec, sec_ct sec, TSome (T_i32 sec)] (cts'@ [ct1,ct2,ct3])"
  ...
  hence "ct_eq ct3 (TSome (T_i32 sec))"
...
lemma type_update_select_type_length3:
  assumes "type_update_select sec (Type tn) = (Type tm)"
  shows "\<exists>t ts'. tn = ts'@[t, t, (T_i32 sec)]"
proof -
  have tn_cond:"(length tn \<ge> 3 \<and> (tn!(length tn-2)) = (tn!(length tn-3)))"
    using assms
    by (simp, metis checker_type.distinct(5))
  hence tm_def:"consume (Type tn) [TAny, TSome (T_i32 sec)] = Type tm"
    using assms
  ...
  hence t1_t2_eq:"t1 = t2"
    ...
    by (metis (no_types, lifting) Suc_diff_Suc Suc_eq_plus1_left Suc_lessD tn''_def
                                  ...
                                  zero_less_diff)
  have "ct_suffix [TAny, TSome (T_i32 sec)] (to_ct_list (tn' @ [t1, t2, t3]))"
    using tn_def tm_def
    by (simp, metis checker_type.distinct(5))
  hence "t3 = (T_i32 sec)"
    using ct_suffix_unfold_one[of "[TAny]" "TSome (T_i32 sec)" "to_ct_list (tn' @ [t1, t2])" "TSome t3"]
          ct_eq.simps(1)
    unfolding to_ct_list_def
    by simp
  thus ?thesis
...
lemma select_return_top_exists:
  assumes "select_return_top sec cts c1 c2 = ctm"
          "ctm \<noteq> Bot"
  shows "\<exists>xs. ctm = TopType xs"
  using assms
  by (cases c1; cases c2) (auto split: if_splits)

lemma type_update_select_top_exists:
  assumes "type_update_select sec xs = (TopType tm)"
...
(*
lemma c:
  assumes "ct_suffix [TSome (T_i32 sec)] cts"
          "length cts \<ge> 3"
  shows "type_update_select sec (TopType cts) = (select_return_top sec cts (cts!(length cts-2)) (cts!(length cts-3)))"
proof -
  obtain nat where nat_def:"length cts = Suc (Suc (Suc nat))"
    using assms(2)
    by (metis add_eq_if diff_Suc_1 le_Suc_ex numeral_3_eq_3 nat.distinct(2))
  have "ct_suffix [TAny, TAny, TSome (T_i32 sec)] cts"
    using assms ct_suffix_extend_any1
    by simp
  then obtain cts' where "consume (TopType cts) [TAny, TAny, TSome (T_i32 sec)] = TopType cts'"
    by simp
  thus ?thesis
    using nat_def select_return_top_exists
    apply (cases "select_return_top sec cts (cts ! Suc nat) (cts ! nat)")
      apply simp_all
    apply blast
    done
qed *)
...
lemma select_return_top_ct_eq:
  assumes "select_return_top sec cts c1 c2 = TopType ctm"
          "length cts \<ge> 3"
  ...
  using assms ct_suffix_cons_ct_list_single[of "take (length cts - 3) cts"]
  by (cases c1; cases c2) (fastforce split: if_splits)+

lemma ens_sec_ct_imp_ct_eq:
  assumes "ct_eq (ens_sec_ct sec ct) ct'"
  shows "ct_eq ct ct'"
  using assms
  apply (cases sec; cases ct)
  apply simp_all
  using can_secret_ct.simps(2) ct_eq.elims(3)
  by blast

lemma ens_sec_ct_imp_ct_eq_sec:
  assumes "ct_eq ct ct'"
          "sec = Secret \<longrightarrow> can_secret_ct ct'"
  shows "ct_eq (ens_sec_ct sec ct) ct'"
  using assms
  apply (cases sec; cases ct)
  by simp_all
  
lemma ct_eq_TSecret_imp_is_secret_t:
  assumes "ct_eq ct1 TSecret"
          "ct_eq (ens_sec_ct Secret ct1) (TSome t'')"
  shows "is_secret_t t''"
  using assms
  by (cases ct1) auto

lemma ct_eq_TSome_imp_ct_eq_TSecret:
  assumes "ct_eq ct (TSome t)"
          "(sec = Secret \<longrightarrow> is_secret_t t)"
  shows "ct_eq ct (sec_ct sec)"
  using assms
  apply (cases ct)
  apply simp_all
  using can_secret_ct.simps(2) ct_eq.elims(3) apply blast
  apply (metis (full_types) can_secret_ct.simps(3) checker_type.simps(6) sec_ct.elims select_return_top.simps(2) select_return_top_sec)
  by (metis can_secret_ct.simps(1) ct_eq.simps(2) ct_eq.simps(7) ct_eq_commute sec_ct.elims)
(* TODO: neater *)

lemma select_return_top_secret:
  assumes "select_return_top Secret ts ct1 ct2 = ct3"
          "ct3 \<noteq> Bot"
          "c_types_agree ct3 t3"
  shows "is_secret_t (last t3)"
  using assms
  apply (cases ct1; cases ct2)
  using ct_suffix_cons_ct_list_single apply force
  using ct_suffix_cons_ct_list_single apply fastforce
  apply (metis c_types_agree.simps(2) ct_eq.simps(4) ct_eq_TSecret_imp_is_secret_t ct_suffix_cons_ct_list_single last_snoc select_return_top.simps(3))
  using ct_suffix_cons_ct_list_single apply force
  using ct_suffix_cons_ct_list_single apply fastforce
  apply (metis (no_types, lifting) c_types_agree.elims(2) can_secret_ct.simps(1) checker_type.distinct(1) checker_type.inject(1) ct_eq.simps(1) ct_suffix_cons_ct_list_single select_return_top.simps(6) snoc_eq_iff_butlast)
  apply (metis c_types_agree.simps(2) ct_eq.simps(4) ct_eq_TSecret_imp_is_secret_t ct_suffix_cons_ct_list_single last_snoc select_return_top.simps(1))
  apply (metis c_types_agree.simps(2) ct_eq.simps(4) ct_eq_TSecret_imp_is_secret_t ct_suffix_cons_ct_list_single last_snoc select_return_top.simps(5))
  by (metis c_types_agree.simps(2) can_secret_ct.simps(1) ct_eq.simps(4) ct_eq_TSecret_imp_is_secret_t ct_suffix_cons_ct_list_single ens_sec_ct.simps(4) last_snoc select_return_top.simps(7))
(* TODO: neater *)
end