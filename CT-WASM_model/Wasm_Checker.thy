...
fun convert_cond :: "t \<Rightarrow> t \<Rightarrow> sx option \<Rightarrow> bool" where
  "convert_cond t1 t2 sx = ((t1 \<noteq> t2) \<and> (t_sec t1 = t_sec t2) \<and> (sx = None) = ((is_float_t t1 \<and> is_float_t t2)
...
| "check_single \<C> (Binop_i t iop) ts = (if is_int_t t \<and> (is_secret_t t \<longrightarrow> safe_binop_i iop)
...
| "check_single \<C> (Testop t _) ts = (if is_int_t t
                                       then type_update ts [TSome t] (Type [T_i32 (t_sec t)])
                                       else Bot)"
| "check_single \<C> (Relop_i t _) ts = (if is_int_t t
                                       then type_update ts [TSome t, TSome t] (Type [T_i32 (t_sec t)])
                                       else Bot)"
...
| "check_single \<C> (Cvtop t1 Convert t2 sx) ts = (if (convert_cond t1 t2 sx)
  ...
  (* reinterpret *)
| "check_single \<C> (Cvtop t1 Reinterpret t2 sx) ts = (if ((t1 \<noteq> t2) \<and> (t_sec t1 = t_sec t2) \<and> t_length t1 = t_length t2 \<and> sx = None)
                                                       then type_update ts [TSome t2] (Type [t1])
                                                       else Bot)"
  (* classify *)
| "check_single \<C> (Cvtop t1 Classify t2 sx) ts = (if (is_int_t t2 \<and> is_public_t t2 \<and> classify_t t2 = t1 \<and> sx = None)
                                                    then type_update ts [TSome t2] (Type [t1])
                                                    else Bot)"
  (* declassify *)
| "check_single \<C> (Cvtop t1 Declassify t2 sx) ts = (if ((trust_t \<C>) = Trusted \<and> is_int_t t2 \<and> is_secret_t t2 \<and> declassify_t t2 = t1 \<and> sx = None)
                                                    then type_update ts [TSome t2] (Type [t1])
                                                    else Bot)"
...
| "check_single \<C> (Select sec) ts = type_update_select sec ts"
  (* block *)                                 
...
| "check_single \<C> (If (tn _> tm) es1 es2) ts = (if (b_e_type_checker (\<C>\<lparr>label := ([tm] @ (label \<C>))\<rparr>) es1 (tn _> tm)
                                                ...
                                                then type_update ts (to_ct_list (tn@[T_i32 Public])) (Type tm)
                                                else Bot)"
  (* br *)
...
| "check_single \<C> (Br_if i) ts = (if i < length (label \<C>)
                                    then type_update ts (to_ct_list ((label \<C>)!i @ [T_i32 Public])) (Type ((label \<C>)!i))
                                    else Bot)"
  (* br_table *)
| "check_single \<C> (Br_table is i) ts = (case (same_lab (is@[i]) (label \<C>)) of
                                        None \<Rightarrow> Bot
                                      | Some tls \<Rightarrow> type_update ts (to_ct_list (tls @ [T_i32 Public])) (TopType []))"
  (* return *)
...
| "check_single \<C> (Call i) ts = (if i < length (func_t \<C>)
                                    then (case ((func_t \<C>)!i) of
                                            (tr,(tn _> tm)) \<Rightarrow> if (trust_compat (trust_t \<C>) tr)
                                                                 then type_update ts (to_ct_list tn) (Type tm)
                                                                 else Bot)
                                    else Bot)"
  (* call_indirect *)
...
| "check_single \<C> (Set_global i) ts = (if i < length (global \<C>) \<and> is_mut (global \<C> ! i)
  ...
  (* load *)
| "check_single \<C> (Load t tp_sx a off) ts =
                              (case (memory \<C>) of
                                 Some (m, sec) \<Rightarrow>
                                 if t_sec t = sec \<and> load_store_t_bounds a (option_projl tp_sx) t
                                   then type_update ts [TSome (T_i32 Public)] (Type [t])
                                   else Bot
                               | None \<Rightarrow> Bot)"
  (* store *)
| "check_single \<C> (Store t tp a off) ts =
                              (case (memory \<C>) of
                                 Some (m, sec) \<Rightarrow>
                                 if t_sec t = sec \<and> load_store_t_bounds a tp t
                                   then type_update ts [TSome (T_i32 Public),TSome t] (Type [])
                                   else Bot
                               | None \<Rightarrow> Bot)"
  (* current_memory *)
| "check_single \<C> Current_memory ts = (if (memory \<C>) \<noteq> None
                                         then type_update ts [] (Type [T_i32 Public])
                                         else Bot)"
  (* grow_memory *)
| "check_single \<C> Grow_memory ts = (if (memory \<C>) \<noteq> None
                                      then type_update ts [TSome (T_i32 Public)] (Type [T_i32 Public])
                                      else Bot)"
